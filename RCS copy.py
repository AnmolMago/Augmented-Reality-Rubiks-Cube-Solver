...
def main():
    ...
    while True:
        ...
        gray_track = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
        blur = cv2.GaussianBlur(img,(7,7),0)
        gray = cv2.cvtColor(blur,cv2.COLOR_BGR2GRAY)
        if tracking:
            
            # tpoints = np.float32(tpoints).reshape(-1,1,2)
            
            # new_tpoints, status, err = cv2.calcOpticalFlowPyrLK(gray_old, gray, tpoints, None, **lk_params)
            # features = [ p for (st,p) in zip(status, new_tpoints) if st]
            # print len(features)
            # for point in features:
            #     point = point[0]
            #     cv2.circle(img2, tuple(map(lambda x: int(x), point)), 5, (255,0,0), 2)

            # tpoints = new_tpoints
            # gray_old = gray

            if len(tracks) > 0:
                img0, img1 = gray_old, gray_track
            ...
            if lines is not None and len(lines) <= 100:
                ...
                for i in range(0, len(parallelPairs)):
                    for j in range (0, i+1):
                        ...
                        if angle_diff >= math.radians(80) and angle_diff <= math.radians(100) and dist_diff <= 20:
                            ...
                            perpendicularFoursome.append((intersections, expectedPoints))
                ...
                for obj in perpendicularFoursome:
                    intersections, expectedPoints = obj
                    score = 0
                ...
                if topScore >= 3:
                    ...
                    topPoints = np.int0(topPoints)
                    # img = cv2.polylines(img,[topPoints],True, 255,2)
